(function () {
    ...
    // A macrotask tick gives the viewport a moment to settle before we trust
    // window.innerWidth (it can read 0 checked synchronously at parse time in
    // some embeddings) — setTimeout is used instead of requestAnimationFrame
    // because rAF never fires in a backgrounded/hidden tab and would leave
    // the decision stuck forever.
    setTimeout(decide, 0);

    function decide() {
        // Decided before Three.js is ever requested, so mobile/reduced-motion
        // visitors never pay for the library download at all.
        var width = window.innerWidth || document.documentElement.clientWidth || 0;
        var eligible = !reduceMotion && width >= 760 && !!canvas3d && supportsWebGL();
        ...
        script.onerror = function () {
            // CDN failed — fall back to the lightweight 2D canvas instead of a blank hero.
            window.__heroUses3D = false;
            canvas3d.style.display = "none";
            if (canvas2d) {
                canvas2d.style.display = "";
                window.dispatchEvent(new CustomEvent("hero3dfallback"));
            }
        };
